...
const BlobEffect = ({ blobColor }) => {

    const tweenRef = useRef(null);
    const [blobId1, blobId2] = [`blob1_${Math.random().toString(36).substring(7)}`, `blob2_${Math.random().toString(36).substring(7)}`]

    useEffect(() => {
        // Select the elements
        const blob1 = document.querySelector(`#${blobId1}`);
        const blob2 = document.querySelector(`#${blobId2}`);

        // Set up the KUTE.js animation
        ...
        // Clean up the animation on component unmount (optional)
        return () => {
